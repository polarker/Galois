...
namespace gs
{
    ...
    template<typename T>
    void OrderedModel<T>::add_train_dataset(const initializer_list<SP_NArray<T>> data, const SP_NArray<T> target) {
        add_train_dataset(data, {target});
    }

    template<typename T>
    void OrderedModel<T>::add_train_dataset(const SP_NArray<T> data, const initializer_list<SP_NArray<T>> target) {
        add_train_dataset({data}, target);
    }

    template<typename T>
    void OrderedModel<T>::add_train_dataset(const initializer_list<SP_NArray<T>> data, const initializer_list<SP_NArray<T>> target) {
        add_train_dataset(vector<SP_NArray<T>>(data), vector<SP_NArray<T>>(target));
    }

    template<typename T>
    void OrderedModel<T>::add_train_dataset(const vector<SP_NArray<T>>& data, const vector<SP_NArray<T>>& target) {
    ...
    template<typename T>
    void OrderedModel<T>::add_test_dataset(const SP_NArray<T> data, const SP_NArray<T> target) {
        add_test_dataset({data}, {target});
    }

    template<typename T>
    void OrderedModel<T>::add_test_dataset(const initializer_list<SP_NArray<T>> data, const SP_NArray<T> target) {
        add_test_dataset(data, {target});
    }

    template<typename T>
    void OrderedModel<T>::add_test_dataset(const SP_NArray<T> data, const initializer_list<SP_NArray<T>> target) {
        add_test_dataset({data}, target);
    }

    template<typename T>
    void OrderedModel<T>::add_test_dataset(const initializer_list<SP_NArray<T>> data, const initializer_list<SP_NArray<T>> target) {
        add_test_dataset(vector<SP_NArray<T>>(data), vector<SP_NArray<T>>(target));
    }

    template<typename T>
    void OrderedModel<T>::add_test_dataset(const vector<SP_NArray<T>>& data, const vector<SP_NArray<T>>& target) {
        CHECK(input_ids.size() == data.size(), "number of input should be equal");
        CHECK(output_ids.size() == target.size(), "number of output should be equal");
    ...
    template<typename T>
    double OrderedModel<T>::compute_correctness(SP_Signal<T> output_signal) {
    ...
    template class OrderedModel<float>;
    template class OrderedModel<double>;
